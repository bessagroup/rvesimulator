...
def create_sphere(model, sphere_center, radius, index):
    ...
    part.PartitionCellByDatumPlane(
        datumPlane=part.datums[2], cells=part.cells[:])
    part.DatumPlaneByPrincipalPlane(offset=0.0, principalPlane=XZPLANE)
    # create a partition
    part.PartitionCellByDatumPlane(
        datumPlane=part.datums[4], cells=part.cells[:])
    part.DatumPlaneByPrincipalPlane(offset=0.0, principalPlane=XYPLANE)
    # create a partition
    part.PartitionCellByDatumPlane(
        datumPlane=part.datums[6], cells=part.cells[:])

    del sketch
    assembly = model.rootAssembly
    instance = assembly.Instance(
        name=f'Sphere-{index}', part=part, dependent=ON)
    assembly.translate(instanceList=(
        f'Sphere-{index}', ), vector=sphere_center)
...
def find_node_pair(face_nodes_list_1, face_nodes_list_2):
    ...
    distance_matrix = distance.cdist(
        face_nodes_array_1, face_nodes_array_2,  'euclidean')
...
def j2_plastic_3d_rve(dict):
    ...
    hardening_table_matrix = np.zeros(
        (len(dict["hardening_table_matrix"][0]), 2))
    for ii in range(2):
    ...
    hardening_table_fibre = np.zeros(
        (len(dict["hardening_table_fiber"][0]), 2))
    for ii in range(2):
    ...
    part = model.Part(name='Matrix', dimensionality=THREE_D,
                      type=DEFORMABLE_BODY)
    ...
    for ii in range(len(sphere_info)):
        sphere_center = (sphere_info[ii][0],
                         sphere_info[ii][1], sphere_info[ii][2])
        shpere_instance = create_sphere(
            model, sphere_center, sphere_info[ii][3], ii)
    ...
    part = model.Part(name='Matrix_cut_xoy',
    ...
    assembly = model.rootAssembly
    instance = assembly.Instance(
        name='Matrix_cut_xoy', part=part, dependent=ON)
    # translate the part to the right position
    assembly.translate(instanceList=(f'Matrix_cut_xoy', ),
                       vector=(0, 0, -0.5*height))
    ...
    # translate the part to the right position
    assembly.translate(instanceList=('Matrix_cut_xoy-rotate', ),
                       vector=(0.0, 1.5*height, 0.0))
    ...
    part.Set(
        cells=(c[max_volume_cell_index:max_volume_cell_index+1],), name='Matrix')
    ...
    model.materials['fiber'].Elastic(
        table=((young_modulus_fibre, poisson_ratio_fibre), ))
    ...
    model.materials['matrix'].Elastic(
        table=((young_modulus_matrix, poisson_ratio_matrix), ))
    ...
    RF_X_id = assembly.ReferencePoint(
        point=(RVEcenter[0] + length, RVEcenter[1], RVEcenter[2])).id
    RF_Y_id = assembly.ReferencePoint(
        point=(RVEcenter[0], RVEcenter[1] + width, RVEcenter[2])).id
    RF_Z_id = assembly.ReferencePoint(
        point=(RVEcenter[0], RVEcenter[1], RVEcenter[2] + height)).id
    ...
    assembly.SetFromNodeLabels(name='vertex_FRB', nodeLabels=(
        ('RVE', (vertex_FRB[0].label,)),), unsorted=True)
    vertex_FRT = part.sets['vertex_FRT'].nodes
    assembly.SetFromNodeLabels(name='vertex_FRT', nodeLabels=(
        ('RVE', (vertex_FRT[0].label,)),), unsorted=True)
    vertex_FLT = part.sets['vertex_FLT'].nodes
    assembly.SetFromNodeLabels(name='vertex_FLT', nodeLabels=(
        ('RVE', (vertex_FLT[0].label,)),), unsorted=True)
    vertex_FLB = part.sets['vertex_FLB'].nodes
    assembly.SetFromNodeLabels(name='vertex_FLB', nodeLabels=(
        ('RVE', (vertex_FLB[0].label,)),), unsorted=True)
    vertex_BLB = part.sets['vertex_BLB'].nodes
    assembly.SetFromNodeLabels(name='vertex_BLB', nodeLabels=(
        ('RVE', (vertex_BLB[0].label,)),), unsorted=True)
    vertex_BLT = part.sets['vertex_BLT'].nodes
    assembly.SetFromNodeLabels(name='vertex_BLT', nodeLabels=(
        ('RVE', (vertex_BLT[0].label,)),), unsorted=True)
    vertex_BRT = part.sets['vertex_BRT'].nodes
    assembly.SetFromNodeLabels(name='vertex_BRT', nodeLabels=(
        ('RVE', (vertex_BRT[0].label,)),), unsorted=True)
    vertex_BRB = part.sets['vertex_BRB'].nodes
    assembly.SetFromNodeLabels(name='vertex_BRB', nodeLabels=(
        ('RVE', (vertex_BRB[0].label,)),), unsorted=True)
    ...
    edgesFRONT_RIGHT_nodes_sorted = sorted(
        edgesFRONT_RIGHT_nodes, key=get_node_z)
    ...
    if len(edgesFRONT_RIGHT_nodes_sorted) == len(edgesBACK_LEFT_nodes_sorted):
        for ii in range(len(edgesFRONT_RIGHT_nodes_sorted)):
            assembly.SetFromNodeLabels(name='FRONTRIGHT_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesFRONT_RIGHT_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='BACKLEFT_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesBACK_LEFT_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print(
            f"edgesFRONT_RIGHT_nodes: {len(edgesFRONT_RIGHT_nodes_sorted)}, edgesBACK_LEFT_nodes:{len(edgesBACK_LEFT_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesFRONT_RIGHT_nodes_sorted) - len(edgesBACK_LEFT_nodes_sorted))} nodes from the larger side")
        if len(edgesFRONT_RIGHT_nodes_sorted) > len(edgesBACK_LEFT_nodes_sorted):
            num_remove = len(edgesFRONT_RIGHT_nodes_sorted) - \
                len(edgesBACK_LEFT_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesFRONT_RIGHT_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesFRONT_RIGHT_nodes_sorted.pop(index_remove[jj])

        else:
            num_remove = len(edgesBACK_LEFT_nodes_sorted) - \
                len(edgesFRONT_RIGHT_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesBACK_LEFT_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    edgesFRONT_LEFT_nodes_sorted = sorted(
        edgesFRONT_LEFT_nodes, key=get_node_z)
    ...
    edgesBACK_RIGHT_nodes_sorted = sorted(
        edgesBACK_RIGHT_nodes, key=get_node_z)
    ...
    if len(edgesFRONT_LEFT_nodes_sorted) == len(edgesBACK_RIGHT_nodes_sorted):
        for ii in range(len(edgesFRONT_LEFT_nodes_sorted)):
            assembly.SetFromNodeLabels(name='FRONTLEFT_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesFRONT_LEFT_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='BACKRIGHT_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesBACK_RIGHT_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print("The number of nodes between the two sides are not the same")
        print(
            f"edgesFRONT_LEFT_nodes: {len(edgesFRONT_LEFT_nodes_sorted)}, edgesBACK_RIGHT_nodes:{len(edgesBACK_RIGHT_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesFRONT_LEFT_nodes_sorted) - len(edgesBACK_RIGHT_nodes_sorted))} nodes from the larger side")
        if len(edgesFRONT_LEFT_nodes_sorted) > len(edgesBACK_RIGHT_nodes_sorted):
            num_remove = len(edgesFRONT_LEFT_nodes_sorted) - \
                len(edgesBACK_RIGHT_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesFRONT_LEFT_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesFRONT_LEFT_nodes_sorted.pop(index_remove[jj])
        else:
            num_remove = len(edgesBACK_RIGHT_nodes_sorted) - \
                len(edgesFRONT_LEFT_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesBACK_RIGHT_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    edgesBACK_BOTTOM_nodes_sorted = sorted(
        edgesBACK_BOTTOM_nodes, key=get_node_y)
    # pop the first and last node (those are the vertices)
    ...
    if len(edgesFRONT_TOP_nodes_sorted) == len(edgesBACK_BOTTOM_nodes_sorted):
        for ii in range(len(edgesFRONT_TOP_nodes_sorted)):
            assembly.SetFromNodeLabels(name='FRONTTOP_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesFRONT_TOP_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='BACKBOTTOM_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesBACK_BOTTOM_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print("The number of nodes between the two sides are not the same")
        print(
            f"edgesFRONT_TOP_nodes: {len(edgesFRONT_TOP_nodes_sorted)}, edgesBACK_BOTTOM_nodes:{len(edgesBACK_BOTTOM_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesFRONT_TOP_nodes_sorted) - len(edgesBACK_BOTTOM_nodes_sorted))} nodes from the larger side")
        if len(edgesFRONT_TOP_nodes_sorted) > len(edgesBACK_BOTTOM_nodes_sorted):
            num_remove = len(edgesFRONT_TOP_nodes_sorted) - \
                len(edgesBACK_BOTTOM_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesFRONT_TOP_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesFRONT_TOP_nodes_sorted.pop(index_remove[jj])
        else:
            num_remove = len(edgesBACK_BOTTOM_nodes_sorted) - \
                len(edgesFRONT_TOP_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesBACK_BOTTOM_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    edgesFRONT_BOTTOM_nodes_sorted = sorted(
        edgesFRONT_BOTTOM_nodes, key=get_node_y)
    ...
    if len(edgesFRONT_BOTTOM_nodes_sorted) == len(edgesBACK_TOP_nodes_sorted):
        for ii in range(1, len(edgesFRONT_BOTTOM_nodes_sorted)):
            assembly.SetFromNodeLabels(name='FRONTBOTTOM_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesFRONT_BOTTOM_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='BACKTOP_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesBACK_TOP_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print("The number of nodes between the two sides are not the same")
        print(
            f"edgesFRONT_BOTTOM_nodes: {len(edgesFRONT_BOTTOM_nodes_sorted)}, edgesBACK_TOP_nodes:{len(edgesBACK_TOP_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesFRONT_BOTTOM_nodes_sorted) - len(edgesBACK_TOP_nodes_sorted))} nodes from the larger side")
        if len(edgesFRONT_BOTTOM_nodes_sorted) > len(edgesBACK_TOP_nodes_sorted):
            num_remove = len(edgesFRONT_BOTTOM_nodes_sorted) - \
                len(edgesBACK_TOP_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesFRONT_BOTTOM_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesFRONT_BOTTOM_nodes_sorted.pop(index_remove[jj])
        else:
            num_remove = len(edgesBACK_TOP_nodes_sorted) - \
                len(edgesFRONT_BOTTOM_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesBACK_TOP_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    edgesLEFT_BOTTOM_nodes_sorted = sorted(
        edgesLEFT_BOTTOM_nodes, key=get_node_x)
    ...
    if len(edgesRIGHT_TOP_nodes_sorted) == len(edgesLEFT_BOTTOM_nodes_sorted):
        for ii in range(len(edgesRIGHT_TOP_nodes_sorted)):
            assembly.SetFromNodeLabels(name='RIGHTTOP_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesRIGHT_TOP_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='LEFTBOTTOM_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesLEFT_BOTTOM_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print("The number of nodes between the two sides are not the same")
        print(
            f"edgesRIGHT_TOP_nodes: {len(edgesRIGHT_TOP_nodes_sorted)}, edgesLEFT_BOTTOM_nodes:{len(edgesLEFT_BOTTOM_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesRIGHT_TOP_nodes_sorted) - len(edgesLEFT_BOTTOM_nodes_sorted))} nodes from the larger side")
        if len(edgesRIGHT_TOP_nodes_sorted) > len(edgesLEFT_BOTTOM_nodes_sorted):
            num_remove = len(edgesRIGHT_TOP_nodes_sorted) - \
                len(edgesLEFT_BOTTOM_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesRIGHT_TOP_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesRIGHT_TOP_nodes_sorted.pop(index_remove[jj])
        else:
            num_remove = len(edgesLEFT_BOTTOM_nodes_sorted) - \
                len(edgesRIGHT_TOP_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesLEFT_BOTTOM_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    edgesRIGHT_BOTTOM_nodes_sorted = sorted(
        edgesRIGHT_BOTTOM_nodes, key=get_node_x)
    ...
    if len(edgesRIGHT_BOTTOM_nodes_sorted) == len(edgesLEFT_TOP_nodes_sorted):
        for ii in range(len(edgesRIGHT_BOTTOM_nodes_sorted)):
            assembly.SetFromNodeLabels(name='RIGHTBOTTOM_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesRIGHT_BOTTOM_nodes_sorted[ii].label])),), unsorted=True)
            assembly.SetFromNodeLabels(name='LEFTTOP_' + str(ii), nodeLabels=(
                ('RVE', tuple([edgesLEFT_TOP_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1, 4):
        ...
        print("The number of nodes between the two sides are not the same")
        print(
            f"edgesRIGHT_BOTTOM_nodes: {len(edgesRIGHT_BOTTOM_nodes_sorted)}, edgesLEFT_TOP_nodes:{len(edgesLEFT_TOP_nodes_sorted)}")
        print(
            f"Randomly to remove {np.abs(len(edgesRIGHT_BOTTOM_nodes_sorted) - len(edgesLEFT_TOP_nodes_sorted))} nodes from the larger side")
        if len(edgesRIGHT_BOTTOM_nodes_sorted) > len(edgesLEFT_TOP_nodes_sorted):
            num_remove = len(edgesRIGHT_BOTTOM_nodes_sorted) - \
                len(edgesLEFT_TOP_nodes_sorted)
            # randomly generate the index to remove
            index_remove = np.random.choice(
                len(edgesRIGHT_BOTTOM_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
                edgesRIGHT_BOTTOM_nodes_sorted.pop(index_remove[jj])
        else:
            num_remove = len(edgesLEFT_TOP_nodes_sorted) - \
                len(edgesRIGHT_BOTTOM_nodes_sorted)
            index_remove = np.random.choice(
                len(edgesLEFT_TOP_nodes_sorted) - num_remove, num_remove, replace=False)
            for jj in range(len(index_remove)):
    ...
    if len(FRONT_nodes) == len(BACK_nodes):
        ...
        for ii in range(len(FRONT_nodes)):
            assembly.SetFromNodeLabels(name='FRONT_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([FRONT_nodes[mapping_pairs[ii][0]].label])),),
                                       unsorted=True)
            assembly.SetFromNodeLabels(name='BACK_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([BACK_nodes[mapping_pairs[ii][1]].label])),),
                                       unsorted=True)
            for jj in range(1, 4):
        ...
        print(
            f"Randomly to remove {np.abs(len(FRONT_nodes) - len(BACK_nodes))} nodes from the larger side")
        num_remove = len(FRONT_nodes) - len(BACK_nodes)
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(FRONT_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
        ...
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(BACK_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
        ...
        raise ValueError(
            "Something wrong with the number of nodes on the two faces")
    ...
    if len(RIGHT_nodes) == len(LEFT_nodes):
        ...
        for ii in range(len(RIGHT_nodes)):
            assembly.SetFromNodeLabels(name='RIGHT_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([RIGHT_nodes[mapping_pairs[ii][0]].label])),),
                                       unsorted=True)
            assembly.SetFromNodeLabels(name='LEFT_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([LEFT_nodes[mapping_pairs[ii][1]].label])),),
                                       unsorted=True)
            for jj in range(1, 4):
        ...
        print(
            f"Randomly to remove {np.abs(len(RIGHT_nodes) - len(LEFT_nodes))} nodes from the larger side")
        num_remove = len(RIGHT_nodes) - len(LEFT_nodes)
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(RIGHT_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
        ...
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(LEFT_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
    ...
    if len(TOP_nodes) == len(BOTTOM_nodes):
        ...
        for ii in range(len(TOP_nodes)):
            assembly.SetFromNodeLabels(name='TOP_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([TOP_nodes[mapping_pairs[ii][0]].label])),),
                                       unsorted=True)
            assembly.SetFromNodeLabels(name='BOTTOM_' + str(ii),
                                       nodeLabels=(
                                           ('RVE', tuple([BOTTOM_nodes[mapping_pairs[ii][1]].label])),),
                                       unsorted=True)
            for jj in range(1, 4):
        ...
        print(
            f"Randomly to remove {np.abs(len(TOP_nodes) - len(BOTTOM_nodes))} nodes from the larger side")
        num_remove = len(TOP_nodes) - len(BOTTOM_nodes)
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(TOP_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
        ...
        # randomly generate the index to remove
        index_remove = np.random.choice(
            len(BOTTOM_nodes) - num_remove, num_remove, replace=False)
        for jj in range(len(index_remove)):
    ...
    if strain_amplitude is None:
        ...
        for ii, name in enumerate(names):
            ...
            create_random_path(
                model=model, path_table=path_table_temp, path_name=name)
...
def post_process(dict):
    ...
    for ii in range(total_frames):
        ...
        U_ref_x[ii] = np.array(fo['U'].getSubset(
            region=ref_x_node_set).values[0].data)
        U_ref_y[ii] = np.array(fo['U'].getSubset(
            region=ref_y_node_set).values[0].data)
        U_ref_z[ii] = np.array(fo['U'].getSubset(
            region=ref_z_node_set).values[0].data)
        # get the reaction forces
        RF_ref_x[ii] = np.array(fo['RF'].getSubset(
            region=ref_x_node_set).values[0].data)
        RF_ref_y[ii] = np.array(fo['RF'].getSubset(
            region=ref_y_node_set).values[0].data)
        RF_ref_z[ii] = np.array(fo['RF'].getSubset(
            region=ref_z_node_set).values[0].data)
