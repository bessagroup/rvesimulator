...
class PnasSVE:
    ...
    def update_sim_info(
        ...
        location_info = self.location_information(
            size=size, radius=radius, benchmark=benchmark
        )
        ...
        self.sim_info = {
            ...
            "E_matrix": E_matrix,
            "Pr_matrix": Pr_matrix,
            "yield_table_matrix": yield_table,
            "E_fiber": E_fiber,
            "Pr_fiber": Pr_fiber,
    ...
    def location_information(
        size: float, radius: float, benchmark: str = "benchmark_1"
    ) -> list:
        """generate the fiber location for sve
        ...
        benchmark : str, optional
            string to indicate the locations, by default "benchmark_1"
        ...
        if radius >= size / 2:
            raise Exception(
                "The radius should smaller than half of the size \n"
            )
        # generate fiber location
        if benchmark == "benchmark_1":
            location_information = [
                [size / 4, 3 * size / 4, 1],
                [3 * size / 4, size / 4, 1],
            ]
        elif benchmark == "benchmark_2":
            location_information = [
                [size / 4, size / 2, 1],
                [3 * size / 4, size / 2, 1],
            ]
