...
from MicroStrucGenerator import HollowPlate
...
class HollowPlateRVE(RVE2DBase):
    ...
    def __init__(self, sim_info=None):
        ...
        self.mesh_size = min(self.length, self.width) / sim_info["mesh_portion"]
    ...
    def _create_part(self):
        PartGenerator = HollowPlate(point1=(-self.length / 2, -self.width / 2),
                                    point2=(self.length / 2, self.width / 2),
                                    center=self.center, radius=self.radius, model=self.model, name_part=self.part_name)
    ...
    def _create_material(self, name_faces):
        ...
        self.material = MaterialGenerator.CreateElasticMaterial(E=self.youngs_modulus, v=self.poission_ratio)
    ...
    def _create_step(self, initialInc=0.1, maxInc=1, maxNumInc=1000000, minInc=1e-20, timePeriod=1):
        ...
        self.model.StaticStep(initialInc=initialInc, maxInc=maxInc, maxNumInc=maxNumInc, minInc=minInc, name='Step-1',
                              previous='Initial', timePeriod=timePeriod)

        ## create Final-outputs
        self.model.fieldOutputRequests['F-Output-1'].setValues(
            variables=('S', 'E', 'LE', 'ENER', 'ELEN', 'ELEDEN', 'EVOL', 'IVOL'), timeInterval=0.1)
        self.model.FieldOutputRequest(name='F-Output-2', createStepName='Step-1', variables=('U', 'RF'),
                                      timeInterval=0.1)
