...
def uniaxial_test(uniaxial_strain):
    ...
    gl_strain = jnp.array([0, 0, 0, uniaxial_strain]).reshape(2, 2)
...
def get_uniaxial_test_data():
    uniaxial_strains_1 = jnp.linspace(-0.3, 0, 30)
    uniaxial_strains_2 = jnp.linspace(0, 1.5, 20)
    uniaxial_strains = jnp.concatenate((uniaxial_strains_1, uniaxial_strains_2))
    data_samples = jax.vmap(uniaxial_test)(uniaxial_strains)
    return data_samples

def get_shear_test_data():
    shear_strains = jnp.linspace(-0.3, 0.3, 50)
...
if __name__ == '__main__':
    ...
    for vf, seed in zip(volume_fractions, seed_for_volume_fractions):
        ...
        input_data_space_1 = get_uniaxial_test_data()
        # input_data_space_2 = get_shear_test_data()
        ...
        input_data_space_1 = np.hstack((np.full((input_data_space_1.shape[0], 1), vf),
                                        np.full((input_data_space_1.shape[0], 1), seed),
                                        np.full((input_data_space_1.shape[0], 1), "uniaxial"),
                                        input_data_space_1))
        
        # input_data_space_2 = np.hstack((np.full((input_data_space_2.shape[0], 1), vf),
        #                                 np.full((input_data_space_2.shape[0], 1), seed),
        #                                 np.full((input_data_space_2.shape[0], 1), "shear"),
        #                                 input_data_space_2))
        
        # input_data_space = np.vstack((input_data_space_1, input_data_space_2))
        input_data_space = input_data_space_1

    
        
        input_dataframe = pd.DataFrame(input_data_space,
                                    columns=['vol_req', 'seed', 'test', 'dU11', 'dU12', 'dU21', 'dU22']).round(4)
    ...
    full_input_dataframe.to_csv('doe_rubber_cb_all_vf_tension_y.csv', index=True)
